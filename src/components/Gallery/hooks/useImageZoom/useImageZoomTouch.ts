    ...
    const handleTouchEnd = React.useCallback(
        (event: React.TouchEvent) => {
            ...
            if (touchCount === 1 && touchState.touchCount === 1) {
                ...
                if (touchState.touchStartPosition) {
                    ...
                    if (distance < DRAG_THRESHOLD_PX) {
                        ...
                        if (isDoubleTap(currentTime, touchPos)) {
                            ...
                            // Don't stop propagation - let gallery handle single taps if needed
                            setTouchState((prev) => ({
                                ...prev,
                        ...
                        // This was a drag - stop propagation if zoomed
                        if (scaleRef.current > MIN_SCALE) {
                            event.stopPropagation();
                        }
                ...
                // Multi-touch or other gesture ended - always stop propagation
                event.stopPropagation();
        ...
        [enabled, touchState, isDoubleTap, scaleRef, setScale, setPosition],
    );
